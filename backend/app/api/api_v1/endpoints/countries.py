...
def get_countries_with_packages(
    ...
    featured: bool = Query(False, description="Filter to countries with featured packages"),
) -> Any:
    """
    Retrieve countries that have active packages, optionally filtered by package_type and featured status.
    """
    countries = country_service.get_countries_with_packages(db, skip=skip, limit=limit, package_type=package_type, featured_only=featured)
    return [CountryWithRegionResponse.from_orm(country) for country in countries]
...
def update_country(
    *,
    ...
    invalidate_cache_pattern(f"*get_country_details_by_slug*")
    invalidate_cache_pattern(f"*get_countries*")
    return CountryResponse.from_orm(country)
