...
def test_get_media_assets(client: TestClient, db: Session, token_headers):
    ...
    with patch('app.services.media.media_service.get_presigned_url') as mock_get_url:
        ...
        assert len(assets) >= 2  # There might be other assets in the database
        
        # Find our test assets in the response
        asset1 = next((a for a in assets if a["filename"] == "image1.jpg"), None)
        asset2 = next((a for a in assets if a["filename"] == "image2.png"), None)
        
        assert asset1 is not None
        assert asset2 is not None
        assert asset1["file_path"] == "/uploads/123e4567-e89b-12d3-a456-426614174000.jpg"
        assert asset2["file_path"] == "/uploads/223e4567-e89b-12d3-a456-426614174000.png"
        assert asset1["url"] == f"https://example.com/media/{media1.id}"
        assert asset2["url"] == f"https://example.com/media/{media2.id}"
...
def test_delete_media_asset(client: TestClient, db: Session, token_headers):
    ...
    # Store the media asset for later verification
    media_id = media.id
    media_filename = media.filename
    media_file_path = media.file_path
    media_storage_key = media.storage_key
    ...
    try:
        ...
        assert response.status_code == 200
        asset = response.json()
        assert asset["filename"] == media_filename
        assert asset["file_path"] == media_file_path
        assert asset["storage_key"] == media_storage_key
