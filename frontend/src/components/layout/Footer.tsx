...
const Footer: React.FC<FooterProps> = ({
  ...
  const dynamicSections = React.useMemo(() => {
    ...
    // Update Destinations section - show regions
    ...
    if (destinationsIndex !== -1 && regionsWithCountries) {
      const destinationLinks = regionsWithCountries
        .map(region => ({
          label: region.name,
          path: `/destinations/regions/${region.slug}`
        }))
        .slice(0, 8); // Limit to 8 regions just in case
        ...
        <div className="container mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-1">
              ...
              <p className="text-sm mb-4 font-lato">
                Discover extraordinary destinations and create unforgettable memories with Allbound Vacations.
              </p>
              {socialLinks && (
            ...
            <div className="md:col-span-1 lg:col-span-3 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
              {dynamicSections.map((section, index) => (
                <div key={index}>
                  <h3 className="text-sm font-playfair font-bold text-white tracking-wider uppercase mb-4">{section.title}</h3>
                  <ul className="space-y-2">
                    {section.links.map((link, linkIndex) => (
                      <li key={linkIndex}>
                        <Link
                          to={link.path}
                          className="text-sm font-lato text-gray-400 hover:text-primary transition-colors"
                        >
                          {link.label}
                        </Link>
                      </li>
          ...
          <div className="mt-12 pt-8 border-t border-gray-700/50">
            <p className="text-center text-sm font-lato text-gray-500">
              &copy; {currentYear} Allbound Vacations. All rights reserved.
            </p>
          </div>
        </div>
      </div>
