...
const GroupTripBookingsPage: React.FC = () => {
  const [selectedBooking, setSelectedBooking] = useState<Booking | null>(null);

  // Fetch group trip bookings
  const { data: bookings, isLoading, error } = useQuery<Booking[]>({
    queryKey: ['bookings', 'group-trips'],
    queryFn: async () => {
      const response = await apiClient.get<Booking[]>('/api/v1/bookings/type/group_trip');
      return response;
    },
  });
  ...
  return (
    <>
      <Helmet>
        <title>Group Trip Bookings | AllBounds Admin</title>
      </Helmet>

      <div className="px-4 py-6 sm:px-0">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">Group Trip Bookings</h1>
            <p className="mt-1 text-sm text-gray-600">
              Manage bookings for group adventures
            </p>
          </div>
        </div>
        ...
        <div className="bg-white shadow overflow-hidden sm:rounded-md">
          <div className="px-4 py-5 sm:px-6 border-b border-gray-200">
            <h3 className="text-lg leading-6 font-medium text-gray-900">
              All Group Trip Bookings
            </h3>
            <p className="mt-1 max-w-2xl text-sm text-gray-500">
              A list of all group trip bookings including their status and details.
            </p>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">No group trip bookings have been made yet.</p>
            </div>
          )}
        </div>
...
export default GroupTripBookingsPage;