  ...
  useEffect(() => {
    const fetchHotelTypes = async () => {
      try {
        ...
        setHotelTypes(response as HotelType[]);
        setLoading(false);
      } catch (err) {
        setError('Failed to fetch hotel types');
        setLoading(false);
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-center mb-6">
          <div className="sm:flex-auto">
            <h1 className="text-2xl font-playfair text-charcoal">Hotel Types</h1>
            <p className="mt-2 text-sm text-gray-600">
              A list of all hotel types in the system.
            </p>
          </div>
          <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <Link to="/admin/hotel-types/new">
                      ...
                      {hotelTypes.map((hotelType) => (
                        <tr key={hotelType.id}>
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {hotelType.description?.length > 50
                              ? `${hotelType.description.substring(0, 50)}...`
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div className="flex justify-end space-x-2">
                              <Link to={`/admin/hotel-types/${hotelType.id}/edit`}>
...
export default HotelTypesListPage;
