  ...
  useEffect(() => {
    const fetchInclusions = async () => {
      try {
        ...
        setInclusions(response as Inclusion[]);
        setLoading(false);
      } catch (err) {
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="sm:flex sm:items-center mb-6">
          <div className="sm:flex-auto">
            <h1 className="text-2xl font-playfair text-charcoal">Inclusions</h1>
            <p className="mt-2 text-sm text-gray-600">
              A list of all inclusions that can be assigned to packages and group trips.
            </p>
          </div>
          <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
            <Link to="/admin/inclusions/new">
                      ...
                      {inclusions.map((inclusion) => (
                        <tr key={inclusion.id}>
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {inclusion.description?.length > 50
                              ? `${inclusion.description.substring(0, 50)}...`
                          ...
                          <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-500">
                            {inclusion.icon && (
                              <span className="inline-flex items-center">
                                <i className={`fas fa-${inclusion.icon} mr-2`}></i>
                                {inclusion.icon}
                              </span>
                            )}
                          </td>
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <div className="flex justify-end space-x-2">
                              <Link to={`/admin/inclusions/${inclusion.id}/edit`}>
