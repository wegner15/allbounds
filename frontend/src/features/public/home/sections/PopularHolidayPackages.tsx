...
import { Star, Heart, Palmtree, ArrowRight, ChevronLeft, ChevronRight } from 'lucide-react';
...
const PopularHolidayPackages: React.FC = () => {
  ...
  const { data: trips, isLoading, error } = usePopularTrips(activeTab, 'holiday');

  const scrollContainer = (direction: 'left' | 'right') => {
    const container = document.getElementById('popular-holiday-packages-container');
    if (container) {
      ...
      <section className="py-20 bg-slate-50/70 border-b border-gray-100">
        <div className="container mx-auto px-4 max-w-7xl">
          <div className="text-center mb-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-teal/10 text-teal text-xs font-semibold uppercase tracking-wider mb-3">
              <span>🏖️ Leisure & Beach Vacations</span>
            </div>
            <h2 className="text-3xl sm:text-4xl font-serif font-bold text-gray-900">Popular Holiday Packages</h2>
            <p className="text-gray-600 max-w-2xl mx-auto mt-2">
              Focused on leisure and beach destinations, city holidays, luxury island escapes, and international vacations.
            </p>
          </div>
  ...
  if (!locations.length) {
    return null; // Gracefully hide if no holiday packages exist
  }

  return (
    <section className="py-20 bg-slate-50/70 border-b border-gray-100 relative overflow-hidden">
      <div className="container mx-auto px-4 max-w-7xl relative z-10">
        {/* Header */}
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-8">
          <div>
            <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-teal/10 text-teal text-xs font-bold uppercase tracking-wider mb-2.5">
              <span>🏖️ Leisure, Beach & City Escapes</span>
            </div>
            ...
            <p className="text-gray-600 max-w-2xl mt-2 text-sm sm:text-base leading-relaxed">
              Focused on leisure and beach destinations, city holidays, luxury resorts, and worldwide vacation getaways.
            </p>
          </div>
          ...
          >
            {isLoading ? (
              ...
              trips.slice(0, 10).map((trip: any) => (
                ...
                >
                  <Link to={`/packages/${trip.slug}`} className="relative block overflow-hidden">
                    ...
                    {/* Holiday Badge */}
                    <div className="absolute top-3 left-3 bg-teal/95 backdrop-blur-sm text-white text-[11px] font-bold px-2.5 py-1 rounded-full uppercase tracking-wider shadow-sm">
                      🏖️ Holiday
                    </div>
                  ...
                  <div className="p-5 flex flex-col flex-1">
                    <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                      <span className="font-semibold text-teal uppercase tracking-wider">
                    ...
                    <Link to={`/packages/${trip.slug}`}>
                      <h3 className="font-bold text-lg text-gray-900 line-clamp-1 group-hover:text-teal transition-colors">
                        {trip.name}
                      </h3>
                    </Link>
                    ...
                    {trip.conversion_triggers && trip.conversion_triggers.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 mb-4">
                        {trip.conversion_triggers.slice(0, 2).map((trigger: string, i: number) => (
                          <span
                            key={i}
                            className="bg-teal/10 text-teal text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-tighter"
                          >
                            {trigger}
                          </span>
                        ))}
                    ...
                    <div className="mt-auto pt-4 border-t border-gray-100 flex items-center justify-between">
                      ...
                      <Link
                        ...
                        className="px-3.5 py-1.5 bg-teal/10 text-teal hover:bg-teal hover:text-white rounded-lg text-xs font-bold transition-colors"
                      >
                        View Details
                      </Link>
                    </div>
              ...
              <div className="w-full text-center py-12 text-gray-500 bg-white rounded-2xl border border-gray-100">
                No holiday packages currently available for {activeTab}.
              </div>
            )}
          </div>
...
export default PopularHolidayPackages;
